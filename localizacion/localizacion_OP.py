...
def localizacion(balizas, real, ideal, centro, radio, mostrar=0):
  ...
  # Recorre el rango [-radio hasta radio] en función del último valor
  for i in np.arange(-radio, radio, 0.1):   # Primero en un sentido (vertical)
    imagen.append([])
    for j in np.arange(-radio, radio, 0.1): # Luego en el otro sentido (horizontal)
  ...
  if mostrar:
    ...
    plt.plot(balT[0],balT[1],'or',ms=10)                  # balizas rojas
    plt.plot(ideal.x,ideal.y,'D',c='#ff00ff',ms=10,mew=2) # robot ideal en rosa 
    plt.plot(real.x, real.y, 'D',c='#00ff00',ms=10,mew=2) # robot real en verde
    plt.show()
    raw_input()
    plt.clf()
...
## Calculamos el centro para localización
...
for punto in objetivos:
  ...
  # y mejor que 1000 (distancia máximo permitida, entiendo)
